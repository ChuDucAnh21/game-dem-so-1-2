...
export class CircleCheckOverlay {
  ...
  public init() {
    ...
    this.root = this.scene.add.container(0, 0).setDepth(4000).setScrollFactor(0);
    this.root.setVisible(false);

    // lớp chặn click phía dưới
    this.blocker = this.scene.add
      .rectangle(0, 0, w, h, 0x000000, 0.15)
      .setOrigin(0, 0)
      .setScrollFactor(0)
    ...
    // input vẽ lên panel
    this.blocker.on("pointerdown", (p: Phaser.Input.Pointer) => this.onDown(p));
    this.blocker.on("pointermove", (p: Phaser.Input.Pointer) => this.onMove(p));
    this.blocker.on("pointerup", (p: Phaser.Input.Pointer) => this.onUp(p));
    this.blocker.on("pointerupoutside", (p: Phaser.Input.Pointer) => this.onUp(p));
  }

  public show(opts: CircleCheckOverlayOptions) {
    if (!this.root || !this.panel || !this.panelBg || !this.drawGfx || !this.resultGfx) return;
    ...
    this.panelW = w * 0.50;
this.panelH = h * 0.50;   // ✅ cao hơn (0.30 -> 0.50)
this.panelX = w * 0.5;
this.panelY = h * 0.36;   // ✅ hạ xuống để panel không đè banner
    ...
    const positions = this.layoutPositions(rawItems.length, this.rows, this.panelW, this.panelH);
    ...
    (this as any)._successKey = opts.successKey ?? "voice_congrats";
    (this as any)._failKey = opts.failKey ?? "voice_try_again";
  }
  ...
  private layoutPositions(count: number, rows: number, panelW: number, panelH: number) {
  ...
  private autoScaleByPanel() {
    ...
    return Math.max(0.22, Math.min(0.36, this.panelH / (this.scene.scale.height * 0.9)));
  }
...
private isStrokeClosed(points: Phaser.Math.Vector2[]) {
  ...
  if (Phaser.Math.Distance.Between(first.x, first.y, last.x, last.y) <= thr) return true;
  ...
  for (let i = 0; i < points.length - 8; i++) {
    const p = points[i];
    if (Phaser.Math.Distance.Between(p.x, p.y, last.x, last.y) <= thr) return true;
  }
...
private polygonAreaFromPoints(points: Phaser.Math.Vector2[]) {
  ...
  private isInsidePanel(worldX: number, worldY: number) {
    ...
    return worldX >= r.left && worldX <= r.right && worldY >= r.top && worldY <= r.bottom;
  }
  ...
  if (!closed) {
    ...
    if (!this.playVoiceSafe(failKey)) this.audio?.play?.("sfx-wrong", { volume: 0.05 });
  ...
  if (isCorrect) {
    if (!this.playVoiceSafe(successKey)) this.audio?.play?.("sfx-correct", { volume: 0.9 });
  ...
  private playVoiceSafe(key: string) {
    if (!key) return false;
    if (this.audio?.has?.(key)) {
      // ưu tiên prompt/feedback tuỳ hệ của bạn
      this.audio.playFeedback?.(key);
      return true;
    }
    return false;
  }
